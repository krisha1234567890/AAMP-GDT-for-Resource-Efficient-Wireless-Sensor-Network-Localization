...
class WSNApp(tk.Tk):
    ...
    def on_prune_sensitivity(self):
        ...
        try:
            ...
            ax1.set_title('Pruning Reduction vs. Threshold', fontsize=12, weight='bold')
            ...
            ax2.set_title('Pairs Before/After Pruning', fontsize=12, weight='bold')
            ...
            plt.close(fig)
            self._log(f"Saved sensitivity plot → {png_path}")
        except Exception as e:
    ...
    def _plot_cumulative_bytes_graph(self, cycle_rows):
        ...
        try:
            ...
            plt.xlabel('Flood Cycle', fontsize=16)
            plt.ylabel('Cumulative Bytes Sent', fontsize=16)
    ...
    def on_mae_heatmap_sigma_theta(self):
        ...
        self.footer_str.set("MAE heatmap sweep complete. See exports/ for CSV, PNG, and fig3.txt.")
    ...
    def run_mobility_trial(
        ...
        try:
            if include_plot:
                ...
                ax.grid(True)
                ax.annotate(
                    f"{mobile_count / total_nodes * 100:.0f}% nodes displaced",
                    xy=(1, mean_vals[0]),
                    xycoords="data",
                    xytext=(-0.08, 20),
                ...
                plt.savefig(png_path, dpi=300, bbox_inches="tight")
                plt.close(fig)
                self._log(f"Saved recovery trajectory plot → {png_path}")
...
def main():
    ...
    parser.add_argument(
        "--export-dir",
        type=str,
        help="Directory for exported artifacts (PNG/TXT). Defaults to ./exports.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--no-plot",
        action="store_true",
        help="Skip generating the PNG plot (still writes fig4.txt).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
